...
const ADD_RECRUITER = gql`
    mutation empresa($name: String!, $password: String!, $email: String!) {
        agregarEmpresa(email: $email, name: $name, password: $password) {
            token
        }
    }
`;
...
const RecruiterRegister: React.FC = () => {
    ...
    const [agregarEmpresa] = useMutation(ADD_RECRUITER);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (valores, { resetForm }) => {
            ...
            try {
                const { data } = await agregarEmpresa({
                    variables: {
                        name,
                        email,
                        password
                ...
                console.log(data);
                const token = data.agregarEmpresa.token;
